...
public class EndranglisteSheet extends SuperMeleeSheet implements IEndRangliste {
    ...
    public EndranglisteSheet(WorkingSpreadsheet workingSpreadsheet) {
        ...
        endRanglisteFormatter = new EndRanglisteFormatter(this, getAnzSpaltenInSpieltag(), spielerSpalte, ERSTE_SPIELTAG_SPALTE,
                getKonfigurationSheet());
    ...
    protected void doRun() throws GenerateException {
        ...
        if (NewSheet.from(getWorkingSpreadsheet(), SHEETNAME).pos(DefaultSheetPos.SUPERMELEE_ENDRANGLISTE).tabColor(SHEET_COLOR)
                .setActiv().hideGrid().forceCreate().spielTagPageStyle(spieltagNr).create().isDidCreate()) {
    ...
    private void formatDatenGeradeUngeradeMitStreichSpieltag() throws GenerateException {
        ...
        // Formula fuer sort error, komplette zeile rot einfärben wenn fehler meldung
                ...
                .style(streichSpieltagHintergrundFarbeGeradeStyle).applyNew().
                // ---------------------
                formula1(getFormulastreichSpieltag(false)).operator(ConditionOperator.FORMULA)
                .style(streichSpieltagHintergrundFarbeUnGeradeStyle).applyNew().
                // --------------------------
    ...
    private void formatSchlechtesteSpieltagSpalte() throws GenerateException {
        ...
        Position startStreichspieltag = Position.from(getSchlechtesteSpielTageSpalte(),
                AbstractRanglisteFormatter.ERSTE_KOPFDATEN_ZEILE);
        Position endStreichspieltag = Position.from(startStreichspieltag).zeilePlus(2);

        ColumnProperties columnProperties = ColumnProperties.from().setWidth(MeldungenSpalte.DEFAULT_SPALTE_NUMBER_WIDTH)
        ...
        StringCellValue headerStreichspieltag = StringCellValue.from(getXSpreadSheet(), startStreichspieltag)
                ...
                .setCellBackColor(endRanglisteFormatter.getHeaderFarbe()).setBorder(BorderFactory.from().allBold().toBorder())
                .setComment("Streich-Spieltag").setColumnProperties(columnProperties);
    ...
    private void updateAnzSpieltageSpalte() throws GenerateException {
        ...
        String formula = "=COUNTIF(" + ersteSpielTagErsteZelle.getAddress() + ":" + letzteSpielTagLetzteZelle.getAddress()
        ...
        StringCellValue headerAnzTage = StringCellValue.from(getXSpreadSheet(), start).setEndPosMerge(end)
                .setCharWeight(FontWeight.LIGHT).setRotateAngle(27000).setVertJustify(CellVertJustify2.CENTER).setValue("Tage")
                .setCellBackColor(endRanglisteFormatter.getHeaderFarbe()).setBorder(BorderFactory.from().allBold().toBorder())
                .setComment("Gespielte Tage");

...
public class Lo {
	...
	/*
	 * unsure about these: chart2 "80243D39-6741-46C5-926E-069164FF87BB" service: com.sun.star.chart2.ChartDocument
	 * 
	 * applet "970B1E81-CF2D-11CF-89CA-008029E4B0B1" service: com.sun.star.comp.sfx2.AppletObject
	 * 
	 * plug-in "4CAA7761-6B8B-11CF-89CA-008029E4B0B1" service: com.sun.star.comp.sfx2.PluginObject
	 * 
	 * frame "1A8A6701-DE58-11CF-89CA-008029E4B0B1" service: com.sun.star.comp.sfx2.IFrameObject
	 * 
	 * XML report chart "D7896D52-B7AF-4820-9DFE-D404D015960F" service: com.sun.star.report.ReportDefinition
	 * 
	 */
...
//	} // end of setOOoBean()
	...
	/*
	 * create an interface object of class aType from the named service; uses 'old' XMultiServiceFactory, so a document must have already been loaded/created
	 */
	{
		if (msFactory == null) {
	...
	/*
	 * Creation sequence: remote component content (xcc) --> remote service manager (mcFactory) --> remote desktop (xDesktop) --> component loader (XComponentLoader) Once we have a component loader, we
	 * can load a document. xcc, mcFactory, and xDesktop are stored as static globals.
	 */
	{
	...
	/*
	 * docType (without the ""private:factory/") is: "private:factory/swriter" Writer document "private:factory/simpress" Impress presentation document "private:factory/sdraw" Draw document
	 * "private:factory/scalc" Calc document "private:factory/sdatabase" Base document "private:factory/smath" Math formula document
	 * 
	 * These are not handled: "private:factory/schart" Chart "private:factory/swriter/web" Writer HTML Web document "private:factory/swriter/GlobalDocument" Master document
	 * 
	 * ".component:Bibliography/View1" Bibliography-Edit the bibliography entries
	 * 
	 * ".component:DB/QueryDesign" Database comp ".component:DB/TableDesign" ".component:DB/RelationDesign" ".component:DB/DataSourceBrowser" ".component:DB/FormGridView"
	 */
	 ...
	 * 
	 * The names were obtained from http://www.oooforum.org/forum/viewtopic.phtml?t=71294 and by running my DocInfo application on files.
	 * 
	 * I use the docType to distinguish between the various meanings of the PDF ext.
	 * 
	 * This could be a lot more extensive.
	 * 
	 * Use Info.getFilterNames() to get the filter names for your Office (the last time I tried it, I got back 246 names!)
	 */
	{
		switch (ext) {
		case "doc":
	...
	// cmd does not include the ".uno:" substring; e.g. pass "Zoom" not ".uno:Zoom"
	{
		...
		try {
			 ...
			 */
			/*
			 * DispatchResultEvent res = (DispatchResultEvent) helper.executeDispatch(provider, (".uno:" + cmd), "", 0, props); if (res.State == DispatchResultState.FAILURE)
			 * System.out.println("Dispatch failed for \"" + cmd + "\""); else if (res.State == DispatchResultState.DONTKNOW) System.out.println("Dispatch result unknown for \"" + cmd + "\"");
			 */
	...
	/*
	 * format is: "vnd.sun.star.script:Foo/Foo." + itemName + "?language=Java&location=share";
	 */
	{
	...
	/*
	 * call XInspector.inspect() in the Inspector.oxt extension Available from https://wiki.openoffice.org/wiki/Object_Inspector
	 */
	{
		if ((xcc == null) || (mcFactory == null)) {
			System.out.println("No office connection found");
		...
		try {
			...
			/*
			 * // report on inspector XServiceInfo si = Lo.qi(XServiceInfo.class, inspector); System.out.println("Implementation name: " + si.getImplementationName()); String[] serviceNames =
			 * si.getSupportedServiceNames(); for(String nm : serviceNames) System.out.println("Service name: " + nm);
			 */
			...
			/*
			 * // alternative, low-level way of getting the method Object coreReflect = mcFactory.createInstanceWithContext( "com.sun.star.reflection.CoreReflection", xcc); XIdlReflection idlReflect =
			 * Lo.qi(XIdlReflection.class, coreReflect); XIdlClass idlClass = idlReflect.forName("org.openoffice.XInstanceInspector"); XIdlMethod[] methods = idlClass.getMethods();
			 * System.out.println("No of methods: " + methods.length); for(XIdlMethod m : methods) System.out.println("  " + m.getName());
			 * 
	...
	/*
	 * call MRI's inspect() Available from http://extensions.libreoffice.org/extension-center/mri-uno-object-inspection-tool or http://extensions.services.openoffice.org/en/project/MRI Docs:
	 * https://github.com/hanya/MRI/wiki Forum tutorial: https://forum.openoffice.org/en/forum/viewtopic.php?f=74&t=49294
	 */
	{
		...
		for (int i = 0; i < con.getCount(); i++) {
			try {
				...
				if (named.getName().equals(nm)) {
					return Lo.qi(XPropertySet.class, oElem);
				}
			} catch (Exception e) {

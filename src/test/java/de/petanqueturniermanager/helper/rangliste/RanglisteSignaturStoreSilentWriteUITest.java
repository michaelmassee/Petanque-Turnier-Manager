 ...
 * <p>
 * Hintergrund: {@link RanglisteRefreshListener#pruefeUndStarte} läuft auf dem
 * {@code PTM-RanglisteRefreshDebouncer}-Daemon-Thread. Schrieb der Store dort
 ...
 * <ul>
 *   <li>Alle {@link RanglisteSignaturStore}-Writes feuern <b>kein</b> TurnierEvent.</li>
 ...
 * </ul>
 */
class RanglisteSignaturStoreSilentWriteUITest extends BaseCalcUITest {
    ...
    void storeWritesFeuernKeineTurnierEvents() {
        ...
        try {
            RanglisteSignaturStore.speichereNachRebuild(xDoc, TEST_SCHLUESSEL, "hash-1", "test");
            assertThat(sammler.events())
                    .as("speichereNachRebuild darf kein TurnierEvent triggern")
                    .isEmpty();

            RanglisteSignaturStore.aktualisiereVerifyZeit(xDoc, TEST_SCHLUESSEL);
            assertThat(sammler.events())
                    .as("aktualisiereVerifyZeit darf kein TurnierEvent triggern")
                    .isEmpty();

            RanglisteSignaturStore.markiereRecoveryVersucht(xDoc, TEST_SCHLUESSEL);
            assertThat(sammler.events())
            ...
            RanglisteSignaturStore.speichereNachRebuild(xDoc, TEST_SCHLUESSEL, "hash-2", "test");
            assertThat(sammler.events())

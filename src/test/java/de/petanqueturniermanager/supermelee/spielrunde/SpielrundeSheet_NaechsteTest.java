...
public class SpielrundeSheet_NaechsteTest {
	...
	public void setup() {
		...
		Mockito.when(konfigurationSheetMock.getMaxAnzGespielteSpieltage()).thenReturn(1);
	...
	public void testGespieltenRundenEinlesen() throws Exception {
		...
		for (int teamCntr = 1; teamCntr <= 2; teamCntr++) {
			for (int spielerCntr = 1; spielerCntr <= 3; spielerCntr++) {
				pospielerNr.spalte(
						SpielrundeSheetKonstanten.ERSTE_SPIELERNR_SPALTE + ((teamCntr - 1) * 3) + spielerCntr - 1);
				Mockito
		...
		validateSpieler(meldungen, 3, 4, 8);
		validateSpieler(meldungen, 4, 3, 8);
		validateSpieler(meldungen, 8, 4, 3);

		validateSpieler(meldungen, 12, 6, 9);

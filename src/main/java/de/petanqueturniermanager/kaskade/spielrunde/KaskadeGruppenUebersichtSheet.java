...
/**
 * Kompakte Übersicht "Nr, Name, Gr" für die im Rahmen der Kaskaden-KO-Runden
 * kaskadierend ermittelten Endfelder (A, B, C, D, …). Sortiert nach Team-Nr, der
 * Gruppenbuchstabe wird pro Buchstabe farblich unterschiedlich hervorgehoben.
 */
public class KaskadeGruppenUebersichtSheet extends SheetRunner implements ISheet {
    ...
    public KaskadeGruppenUebersichtSheet(WorkingSpreadsheet workingSpreadsheet) {
        super(workingSpreadsheet, TurnierSystem.KASKADE, "Kaskade-Gruppen-Übersicht");
    ...
    public XSpreadsheet getXSpreadSheet() throws GenerateException {
        return SheetMetadataHelper.findeSheetUndHeile(
                ...
                SheetMetadataHelper.SCHLUESSEL_KASKADE_GRUPPEN_UEBERSICHT,
                SheetNamen.kaskadeGruppenUebersicht());
    }
    ...
    public void generate() throws GenerateException {
        NewSheet.from(this, SheetNamen.kaskadeGruppenUebersicht(),
                SheetMetadataHelper.SCHLUESSEL_KASKADE_GRUPPEN_UEBERSICHT)
                .tabColor(konfigurationSheet.getKaskadenTabFarbe()).pos(DefaultSheetPos.KASKADE_GRUPPEN_UEBERSICHT)
    ...
    private void befuelleGruppenUebersicht() throws GenerateException {
        processBoxinfo("processbox.kaskade.gruppen.uebersicht.einlesen");
        ...
        processBoxinfo("processbox.kaskade.gruppen.uebersicht.einfuegen", eintraege.size());
    ...
    /**
     * Berechnet aus den Kaskadenrunden-Ergebnissen die Endfeld-Zuordnung (Team-Nr → Gruppenbuchstabe).
     */
    private Map<Integer, String> ermittleTeamNrZuGruppe(int gesamtTeams) throws GenerateException {
        boolean freispielGewonnen = konfigurationSheet.getFreispielPunktePlus()
                > konfigurationSheet.getFreispielPunkteMinus();
        var plan = KaskadenKoRundenPlaner.berechne(gesamtTeams, konfigurationSheet.getAnzahlKaskaden(), freispielGewonnen);
        List<KaskadenFeldBelegung> belegungen = new KaskadeRundenErgebnisLeser(getWorkingSpreadsheet())
                .ladeFeldBelegungen(plan);

        Map<Integer, String> teamNrZuGruppe = new HashMap<>();
        for (KaskadenFeldBelegung belegung : belegungen) {
            for (Integer teamNr : belegung.teamNrs()) {
                teamNrZuGruppe.put(teamNr, belegung.bezeichner());
            }
        }
        return teamNrZuGruppe;
    }

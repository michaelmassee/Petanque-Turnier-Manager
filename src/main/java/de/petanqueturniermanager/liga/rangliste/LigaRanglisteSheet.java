...
public class LigaRanglisteSheet extends LigaSheet implements ISheet, IRangliste {
    ...
    public LigaRanglisteSheet(WorkingSpreadsheet workingSpreadsheet) {
        ...
        meldungenSpalte = MeldungenSpalte.Builder().spalteMeldungNameWidth(LIGA_MELDUNG_NAME_WIDTH)
                .ersteDatenZiele(ERSTE_DATEN_ZEILE).spielerNrSpalte(TEAM_NR_SPALTE).sheet(this).formation(Formation.TETE)
                .anzZeilenInHeader(2).build();
    ...
    public void upDateSheet() throws GenerateException {
        ...
        if (!NewSheet.from(this, SHEETNAME).pos(DefaultSheetPos.LIGA_ENDRANGLISTE).setForceCreate(true).setActiv().hideGrid()
    ...
    private void insertHeader() throws GenerateException {
        ...
        StringCellValue header2val = StringCellValue.from(getXSpreadSheet()).setPos(header2Pos).setEndPosMergeSpaltePlus(1)
        ...
        StringCellValue header1val = StringCellValue.from(getXSpreadSheet()).setPos(header1Pos).setEndPosMergeSpaltePlus(5)
                .setCellProperties(headerProp);
        for (int rundeCntr = 0; rundeCntr < anzGesamtRunden; rundeCntr++) {
    ...
    private void formatData() throws GenerateException {
        ...
        GeradeUngeradeFormatHelper.from(this, allDatenRange())
                ...
                .ungeradeFarbe(getKonfigurationSheet().getRanglisteHintergrundFarbeUnGerade()).validateSpalte(validateSpalte())
                .apply();
        ...
        for (int rundeCntr = 0; rundeCntr < anzGesamtRunden; rundeCntr++) {
            RangeHelper.from(this, rundenErsteSpalteRange)
                    .setRangeProperties(RangeProperties.from().setBorder(BorderFactory.from().boldLn().forLeft().toBorder()));
        ...
        RangeHelper
                .from(this, ersteSummeSpalte, ERSTE_DATEN_ZEILE - 3, ersteSummeSpalte + ANZ_SUMMEN_SPALTEN - 1,
                        letzteDatenZeile)
                .setRangeProperties(
                        RangeProperties.from().setBorder(BorderFactory.from().boldLn().forLeft().forRight().toBorder()));
        ...
        RangePosition trennPos = RangePosition.from(ersteSummeSpalte + 2, ERSTE_DATEN_ZEILE - 2, ersteSummeSpalte + 2,
                letzteDatenZeile);
        RangeHelper.from(this, trennPos)
                .setRangeProperties(RangeProperties.from().setBorder(BorderFactory.from().doubleLn().forLeft().toBorder()));
        RangeHelper.from(this, trennPos.spaltePlus(3))
                .setRangeProperties(RangeProperties.from().setBorder(BorderFactory.from().doubleLn().forLeft().toBorder()));
        ...
        for (int summeSpalte = 0; summeSpalte < 6; summeSpalte++) {
            ...
            if (endSummeSpalteOffs == 4) {
                ...
                StringCellValue summenFormulaDiffSpiele = StringCellValue.from(getXSpreadSheet()).setPos(summePos)
                        .spaltePlusEins().setFillAutoDownZeilePlus(autoFillDownZeilePlus).setValue(summenFormulaDiffSpieleStr)
                        .setColumnProperties(columnProperties);
    ...
    private void spieltagEinfuegen(int erstePaarungSpieltagZeileInSpielplan, Position spielTagFormulaPos, int anzPaarungen)
            throws GenerateException {
    ...
    private void verweisAufErgbnisseEinfuegen(int spalteA, int spalteB, int startZeile, int anzPaarungen,
        ...
        StringCellValue spielTagFormula = StringCellValue.from(getXSpreadSheet()).setPos(startFormulaPos)
                .setValue(formulaPunktePlus).setFillAutoDownZeilePlus(anzZeilen() - 1).addColumnProperties(columnProperties);

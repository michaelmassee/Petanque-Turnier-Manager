...
class PouleListeDelegate implements MeldeListeKonstanten {
    ...
    PouleListeDelegate(IMeldeliste<TeamMeldungen, Team> sheet) {
        this.sheet = checkNotNull(sheet);
        konfigurationSheet = new PouleKonfigurationSheet(sheet.getWorkingSpreadsheet());
        meldeListeHelper = new MeldeListeHelper<>(sheet, SheetMetadataHelper.SCHLUESSEL_POULE_MELDELISTE);
    }
    ...
    void formatDatenSpalten() throws GenerateException {
        ...
        if (konfigurationSheet.isMeldeListeTeamnameAnzeigen() && anzSpieler == 0) {
            var teamnameRange = RangePosition.from(1, ERSTE_DATEN_ZEILE, 1, letzteDatenZeile);
            var kondDoppeltTeamname = "AND(NOT(ISBLANK(" + ConditionalFormatHelper.FORMULA_CURRENT_CELL

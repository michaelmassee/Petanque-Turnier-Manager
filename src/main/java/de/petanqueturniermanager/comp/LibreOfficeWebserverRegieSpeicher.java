...
/**
 * Speichert Webserver-Regie-Optionen im LibreOffice-Konfigurationssystem.
 */
final class LibreOfficeWebserverRegieSpeicher {

	private static final Logger logger = LogManager.getLogger(LibreOfficeWebserverRegieSpeicher.class);

	private static final String NODE_PATH = "/org.openoffice.Office.Custom.PetanqueTurnierManager/WebserverRegie";
	...
	LibreOfficeWebserverRegieSpeicher(XComponentContext context) {
		this.context = context;
	}

	WebserverRegieOptionen laden() {
		XPropertySet props = null;
		try {
			props = konfiguration(false);
			return new WebserverRegieOptionen(
					booleanWert(props, PROP_ACTIVE, true),
					portWert(props, PROP_PORT, GlobalProperties.WEBSERVER_REGIE_DEFAULT_PORT),
					stringWert(props, PROP_TARGETS_JSON));
		} catch (Exception e) {
			throw new IllegalStateException("LibreOffice Webserver-Regie-Optionen konnten nicht gelesen werden", e);
		} finally {
			dispose(props);
		}
	}
	...
	boolean istLegacyImportErledigt() {
		...
		try {
			...
			logger.warn("Webserver-Regie-Legacy-Importstatus konnte nicht gelesen werden", e);
			return false;
		} finally {
			dispose(props);
		}
	...
	void speichern(WebserverRegieOptionen optionen) {
		XPropertySet props = null;
		try {
			...
			props.setPropertyValue(PROP_TARGETS_JSON, optionen.zieleJson());
			props.setPropertyValue(PROP_LEGACY_IMPORTED, Boolean.TRUE);
			commit(props);
		} catch (Exception e) {
			throw new IllegalStateException("LibreOffice Webserver-Regie-Optionen konnten nicht gespeichert werden", e);
		} finally {
			dispose(props);
		}
	}

...
/**
 * Speichert die zentrale WhatsApp-Chat-Liste (als JSON) im LibreOffice-Konfigurationssystem.
 */
final class LibreOfficeWhatsAppChatSpeicher {

	private static final Logger logger = LogManager.getLogger(LibreOfficeWhatsAppChatSpeicher.class);

	private static final String NODE_PATH = "/org.openoffice.Office.Custom.PetanqueTurnierManager/WhatsApp";
	private static final String PROP_CHATS_JSON = "ChatsJson";

	private final XComponentContext context;

	LibreOfficeWhatsAppChatSpeicher(XComponentContext context) {
	...
	String laden() {
		...
		try {
			...
			return stringWert(props, PROP_CHATS_JSON);
		} catch (Exception e) {
			throw new IllegalStateException("LibreOffice WhatsApp-Chat-Liste konnte nicht gelesen werden", e);
		} finally {
			dispose(props);
		}
	}

	void speichern(String chatsJson) {
		XPropertySet props = null;
		try {
			props = konfiguration(true);
			props.setPropertyValue(PROP_CHATS_JSON, chatsJson == null ? "" : chatsJson);
			commit(props);
		} catch (Exception e) {
			throw new IllegalStateException("LibreOffice WhatsApp-Chat-Liste konnte nicht gespeichert werden", e);
		} finally {
			dispose(props);
		}
	}

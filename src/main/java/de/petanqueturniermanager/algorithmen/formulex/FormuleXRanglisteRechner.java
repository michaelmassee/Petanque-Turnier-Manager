...
/**
 * Sortiert die Teamergebnisse der Formule-X-Rangliste nach den Auswertungskriterien.
 *
 * <p>Kriterien in absteigender Priorität (siehe turniersysteme/08_Formule_X.md):
 * <ol>
 *   <li>Sieganzahl (absteigend) – Hauptkriterium</li>
 *   <li>Formule-X-Wertungspunkte (absteigend) – Feinwertung</li>
 *   <li>Punktedifferenz (absteigend) – Tie-Breaker</li>
 *   <li>Eigene Punkte (absteigend) – Tie-Breaker</li>
 *   <li>Direktvergleich – nur bei genau 2 gleichrangigen Teams, sofern sie
 *       gegeneinander gespielt haben</li>
 *   <li>Teamnummer (aufsteigend) – „das Los", garantiert stabile, totale Ordnung als Fallback</li>
 * </ol>
 *
 * <p>Bei einem zyklischen Dreier-Gleichstand (A schlägt B, B schlägt C, C schlägt A)
 * ist der Direktvergleich nicht anwendbar. Die Sortierung nach Teamnummer (Kriterium 6)
 * liefert in diesem Fall eine deterministische Ordnung.
 *
 * <p>Der Algorithmus arbeitet in zwei Phasen:
 * <ol>
 *   <li><b>Phase 1:</b> Sortierung nach Kriterien 1–4 + Teamnummer (totale Ordnung).</li>
 ...
 * </ol>
 */
public class FormuleXRanglisteRechner {

    /**
     * Sortiert die übergebenen Teamergebnisse nach den Formule-X-Auswertungskriterien.
     *
     * @param ergebnisse Liste der aggregierten Teamergebnisse (darf leer sein)
     * @return neue, unveränderliche Liste in aufsteigender Platzierungsreihenfolge (Platz 1 zuerst)
     */
    public List<FormuleXTeamErgebnis> sortiere(List<FormuleXTeamErgebnis> ergebnisse) {
        ...
        Comparator<FormuleXTeamErgebnis> basisKomparator =
                Comparator.comparingInt(FormuleXTeamErgebnis::siege).reversed()
                        .thenComparing(Comparator.comparingInt(FormuleXTeamErgebnis::wertung).reversed())
                        .thenComparing(Comparator.comparingInt(FormuleXTeamErgebnis::punktedifferenz).reversed())
                        .thenComparing(Comparator.comparingInt(FormuleXTeamErgebnis::eigenePunkte).reversed())
                        .thenComparingInt(FormuleXTeamErgebnis::teamNr);
     ...
     */
    private void verbessereDurchDirektvergleich(List<FormuleXTeamErgebnis> sortiert,
    ...
    private boolean sindGleichrangig(FormuleXTeamErgebnis a, FormuleXTeamErgebnis b) {
        return a.siege() == b.siege()
                && a.wertung() == b.wertung()
                && a.punktedifferenz() == b.punktedifferenz()
                && a.eigenePunkte() == b.eigenePunkte();
    }

    /**
     * Baut die Paarungen-Matrix für {@link Direktvergleich} auf.
     * Jedes {@link FormuleXTeamErgebnis.SpielErgebnisGegen} ergibt eine Zeile
     * {@code [teamNr, gegnerNr]}.
     */
    private int[][] bauePaarungen(List<FormuleXTeamErgebnis> ergebnisse) {
     ...
     */
    private int[][] baueSiege(List<FormuleXTeamErgebnis> ergebnisse) {
        int anzZeilen = zaehleSpiele(ergebnisse);
     ...
     */
    private int[][] baueSpielPunkte(List<FormuleXTeamErgebnis> ergebnisse) {
        int anzZeilen = zaehleSpiele(ergebnisse);
    ...
    private int zaehleSpiele(List<FormuleXTeamErgebnis> ergebnisse) {

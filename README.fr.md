...
* **Ligue (Championnat) :** Pour l'organisation d'une ligue de club ou régionale avec des équipes fixes. Le calendrier des matchs est généré à l'avance et détermine qui joue contre qui et quand. Le classement est calculé selon les victoires, la différence de boules et la confrontation directe. Prend en charge l'exportation HTML des résultats et du calendrier.
...
* **Élimination directe (Système à élimination directe) :** Pour les phases finales classiques et les tournois à élimination directe pure. Une victoire signifie la qualification, une défaite l'élimination immédiate. Les appariements suivent le système croisé (1er vs Dernier, 2e vs Avant-dernier …), de sorte que les meilleures équipes ne se rencontrent que tardivement. Pour les nombres de participants qui ne sont pas une puissance de deux, une phase de cadrage (tour préliminaire) est automatiquement calculée.
...
* **Trip-Tête / Système Trio :** Format par équipes de trois joueurs dans lequel chaque rencontre comprend une Triplette, une Doublette et un Tête-à-tête. Après la Triplette commune, l'équipe se répartit tactiquement : deux joueurs disputent la Doublette, un joueur dispute le Tête-à-tête. Adapté aux tournois par équipes compacts, variés et avec une notation claire des rencontres.

---

## 👥 Base de joueurs intégrée – saisir une fois, réutiliser indéfiniment

Plutôt que de retaper la liste des participants à chaque tournoi, le Pétanque Tournament Manager intègre une **base de joueurs directement dans l'extension**. Joueurs, clubs et étiquettes sont saisis une seule fois et restent disponibles pour chaque nouveau tournoi.

* 🧑‍🤝‍🧑 **Gestion centralisée des joueurs et clubs** – prénom, nom, club, numéro de licence et étiquettes libres (par ex. « Féminines », « Jeunes », « Club d'origine »).
* 🔍 **Recherche & filtres rapides** – recherche dès 2 caractères, combinable avec les filtres par club et par étiquette.
* ➕ **Reprise directe dans la liste d'inscription** – un double-clic ajoute un joueur ; plusieurs joueurs peuvent être regroupés en équipe et écrits ensemble dans la liste. Les joueurs déjà inscrits peuvent être masqués pour éviter les doublons.
* 📥 **Import / Export** – format CSV et format de fichier DB propre. Partagez l'effectif du club entre tournois ou importez des listes existantes.
* 🌐 **Visualiseur web intégré** – un frontend en lecture seule pour la base de joueurs, idéal pour la table d'inscription ou le bureau du tournoi.

> **💡 Pratique pour les tournois de club :** Saisissez une fois tous les membres actifs – dès le tournoi suivant, il suffit de taper les premières lettres du nom pour ajouter le joueur à la liste avec son club et son numéro de licence.
...
* Tous les systèmes de tournoi pris en charge : Supermêlée, Système suisse, Chacun contre chacun, K.-O., Système maastrichtois, Formule X, Trip-Tête
...
* **Système d'exploitation :** Linux, Windows ou macOS 11 (Big Sur) ou plus récent

---

## ⚙️ Installation & Configuration

Le plus simple est d'utiliser l'**installateur** depuis les [GitHub Releases](https://github.com/michaelmassee/Petanque-Turnier-Manager/releases) (Linux/Windows/macOS) – il configure automatiquement l'extension pour LibreOffice.

Pour le guide complet étape par étape (installation manuelle du `.oxt`, configuration de Java, remarques spécifiques par OS), consultez le **[Wiki d'installation](https://github.com/michaelmassee/Petanque-Turnier-Manager/wiki/Installation)** (en allemand).

...
- **Vollständig auf LO-Konfiguration umgestellt** (kein Legacy-Fallback mehr, Legacy-Keys aktiv aus der Datei
  ausgefiltert): Webserver-Regie, Plugin-/Optionswerte.
- **Primär LO-Konfiguration beim Lesen, mit Legacy-Datei-Fallback** (falls kein LO-Kontext verfügbar, z.B. in
  reinen Unit-Tests ohne laufendes LibreOffice): FTP-Server. Geschrieben wird aber trotzdem immer auch in die
  Legacy-Datei (siehe propMap-Mirror unten), da hierfür (noch) kein aktiver Ausfilter-Mechanismus existiert.
...
"primäre" Quelle die LO-Konfiguration ist (z.B. FTP-Server-JSON), werden beim Laden in dieselbe `propMap`
gespiegelt und landen dadurch bei der nächsten beliebigen Speicherung (z.B. beim Speichern der
Composite-Views) erneut physisch in `PetanqueTurnierManager.properties` — außer für Bereiche, die
gezielt aus dem Schreibpfad ausgefiltert werden: Webserver-Regie (`istWebserverRegieLegacyKey`,
GlobalProperties.java:623) und Plugin-Optionen (`istPluginOptionenLegacyKey`, GlobalProperties.java:660),
jeweils sobald die Migration nach LO erfolgreich war. Die Filterung läuft gebündelt über
`istBereitsInLibreOfficeGespeichert(...)` (GlobalProperties.java:473).
...
- Turnier-Startseite:
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:836 (getStartseitePort), :852 (speichernStartseite)

- Startup-Modus:
    - startup.turnier.modus
    - Keine LO-Konfiguration vorgesehen, ausschließlich Legacy-Datei.
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:878 (isStartupTurnierModus), :882 (setStartupTurnierModus)
...
- Composite-Webserver-Views:
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:898 (getCompositeViewEintraege), :1042 (speichernCompositeViews); src/main/java/de/petanqueturniermanager/webserver/RandKonfiguration.java; src/main/java/de/petanqueturniermanager/comp/CompositeViewsOptionsEventHandler.java
...
- FTP-Server (zentrale Liste):
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:796 (getFtpServerEintraege),
      :811 (speichernFtpServer); src/main/java/de/petanqueturniermanager/comp/LibreOfficeFtpServerSpeicher.java;
...
- Tab-Farben:
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:1173 (getTabFarbe)
...
- Plugin-/Optionswerte:
    ...
    - Legacy-Datei ist Quelle nur bis zur erfolgreichen Migration (`LegacyPropertiesImported=false`). Nach erfolgreichem Import ist die LO-Konfiguration alleinige, persistente Quelle — analog zur Webserver-Regie ist dies die zweite Kategorie, die aktiv aus dem propMap-Vollschreiber ausgefiltert wird (`istPluginOptionenLegacyKey`).
    - Legacy-Keys werden nach erfolgreichem Import einmalig aus der Datei entfernt (`bereinigeLegacyPluginOptionenProperties`) und danach bei jedem Speichern erneut herausgefiltert.
    - Schlägt der LO-Zugriff nach erfolgter Migration ausnahmsweise fehl, greift der Legacy-Datei-Fallback für diese Sitzung wieder (kein bewusster Verzicht wie bei der Regie).
    - Verwaltung über die LO-Optionsseite Extras > Optionen > PétTurnMngr.
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:1140 (speichern), :660 (istPluginOptionenLegacyKey), :639 (bereinigeLegacyPluginOptionenProperties); src/main/java/de/petanqueturniermanager/comp/LibreOfficePluginOptionenSpeicher.java:84
...
- Webserver-Regie:
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:606 (bereinigeLegacyWebserverRegieProperties), :623 (istWebserverRegieLegacyKey), src/main/java/de/petanqueturniermanager/comp/LibreOfficeWebserverRegieSpeicher.java
...
- Alte Einzel-Port-Webserver-Keys werden beim Start entfernt:
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:334 (bereinigeLegacyEinzelPortProperties)

- Webserver-Regie-Legacy-Keys werden nach erfolgreichem LO-Import entfernt:
    - webserver_regie_aktiv
    - webserver_regie_port
    - webserver_regie_ziele
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:606 (bereinigeLegacyWebserverRegieProperties), src/main/java/de/petanqueturniermanager/comp/LibreOfficeWebserverRegieSpeicher.java

- Plugin-Optionen-Legacy-Keys werden nach erfolgreichem LO-Import entfernt:
    - autosave, backup, newversioncheck, prozessbox.automatisch.anzeigen, prozessbox.automatisch.schliessen, performance.logging, loglevel
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:639 (bereinigeLegacyPluginOptionenProperties), src/main/java/de/petanqueturniermanager/comp/LibreOfficePluginOptionenSpeicher.java
...
- Timer-Einstellungen-Legacy-Keys werden beim Start entfernt (keine Wertübernahme):
    ...
    - Code: src/main/java/de/petanqueturniermanager/comp/GlobalProperties.java:361 (bereinigeLegacyTimerProperties), src/main/java/de/petanqueturniermanager/timer/TimerDialog.java

...
- ~~**B1** `build.gradle`: `-Xlint:all -Werror` für Produktions-Source~~ — **erledigt**: aktiviert mit zwei dauerhaften Suppressions (bewusste Architekturentscheidung, kein TODO mehr): `classfile` (externe Lib github-api referenziert nicht-classpath-Annotations, nicht reparierbar) und `this-escape` (~70 Stellen über die Sheet-/Sidebar-Hierarchie; plugin-interne, geschlossene Subklassen-Hierarchien mit etablierter Init-Reihenfolge — Refactoring zu Factory+post-construct hätte ohne UI-CI keinen erkennbaren Robustheits-Gewinn). `unchecked` und `serial` initial mitfixiert (`AbstractStore`, Properties-Klassen, Webserver-Exceptions). `dangling-doc-comments` aufgelöst (~375 Dateien: 350 Header-Kommentare per Sweep `/** … */ → /* … */` migriert, 6 Edge-Cases manuell). `deprecation` aufgelöst (21 Stellen): Apache `StringUtils.equals/equalsIgnoreCase/containsIgnoreCase/compare/replace/appendIfMissing` migriert zu `java.util.Objects.equals` bzw. neuem zentralen `StringTools`-Helper (`equalsIgnoreCase`, `containsIgnoreCase`, `appendIfMissing`); `new URL(String)` → `URI.create(s).toURL()`; `Runtime.exec(String)` → `Runtime.exec(String[])` in `Log4J.openLogFile()`. Test-Source nur Warnings, kein `-Werror`.
